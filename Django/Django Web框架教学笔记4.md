...
##### 3. 查询返回指定列(字典表示)-values('列1', '列2')
    ...
    - 示例:
        ...
        for book in books:
            print("书名", book["title"], '出版社:', book['pub'])
            print("book=", book)
        # 书名 Python高级 出版社: 中信出版社
        # book= {'title': 'Python高级', 'pub': '中信出版社'}
        ```
##### 4. 查询返回指定列（元组表示)-values_list('列1','列2')
...
#### 查询谓词
- 每一个查询谓词是一个独立的查询功能
##### - 一、Author.objects.filter(字段__条件)
...
##### 二. 不等的条件筛选-MyModel.objects.exclude(条件)
    - 语法:
    ...
    - 示例:
        ...
        for book in books:
            print(book)
        ```
##### 三. 查询指定的一条数据-MyModel.objects.get(条件)
    - 语法:
    ...
    - 用法步骤:
        1. 通过先用查询结果MyModel.objects.values 查找查询要分组聚合的列
            ...
            - 如: 
                ...
                print(pub_set)  
                # <QuerySet [{'pub': '清华大学出版社'}, {'pub': '清华大学出版社'}, {'pub_hou {'pub': '机械工业出版社'}, {'pub': '清华大学出版社'}]>
        ...
        2. 通过返回结果的 QuerySet.annotate 方法分组聚合得到分组结果
            ...
            - 如:
                ...
                print(pub_count_set) 
                # <QuerySet [{'pub': '清华大学出版社', 'myCount': 7}, {'pub': '机械工业出版社', 'myCount': 3}]>
                ```
